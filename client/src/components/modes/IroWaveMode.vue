    ...
    <v-card-actions>
      <v-layout row wrap>
        <v-flex fill-height xs12 sm12 md12>
          <iro-color-picker label="__wave.background__" :color="backgroundColor" @colorChange="colorValue => changeColor('backgroundColor', colorValue)"></iro-color-picker>
        </v-flex>
        <v-flex fill-height xs12 sm12 md12>
          <iro-color-picker label="__wave.gradient.start__" :color="gradientStart" @colorChange="colorValue => changeColor('gradientStart', colorValue)"></iro-color-picker>
        </v-flex>
        <v-flex fill-height xs12 sm12 md12>
          <iro-color-picker label="__wave.gradient.end__" :color="gradientEnd" @colorChange="colorValue => changeColor('gradientEnd', colorValue)"></iro-color-picker>
        </v-flex>
      </v-layout>
    </v-card-actions>
    <v-card-actions>
...
export default {
  ...
  methods: {
    ...
    send() {
      ...
      this.$emit('sendToServer', 'wave', params);
    },
  },
